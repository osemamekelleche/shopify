@php
    if (request()->query('theme', '') === 'light') {
        request()->session()->put('dark', false);
    } else if (request()->query('theme', '') === 'dark' || request()->session()->get('dark', false)) {
...
<head>
    ...
    <meta name="csrf-token" content="{{ encrypt(csrf_token(), true) }}">
...
<body>
    <div id="telescope" v-cloak>
        ...
        <div class="container mb-5">
            <div class="d-flex align-items-stretch py-4 header">
                ...
                <router-link to="/monitored-tags" class="btn btn-muted d-flex align-items-center py-2"
                    title="Monitoring">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" class="icon" fill="currentColor">
    ...
    <script>
        ...
        function changeTheme(evt = null) {
            let matches = evt === null ? darkTheme.matches : evt.matches;

            const theme = matches ? 'dark' : 'light';
            window.location.href = window.location.origin + window.location.pathname + `?theme=${theme}`
        }

        const useDarkTheme = @js(request()->session()->get('dark', false));
        /** @type {MediaQueryList} darkTheme */
        const darkTheme = window.matchMedia('(prefers-color-scheme:dark)');
        if (useDarkTheme !== darkTheme.matches) {
            changeTheme();
        }
        ...
        // Removes the query string without reloading the page
